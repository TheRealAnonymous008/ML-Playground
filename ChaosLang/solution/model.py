...
class PhoneticTransformer(nn.Module):
    def __init__(self, vocab, seq_len = 1, languages = {}, pikl : str | None = None):
        super().__init__()
        if pikl == None: 
            self._initialize(vocab, seq_len, languages)
        else:   
            languages = {}          
            with open(pikl, 'rb') as file:
                loaded_data : PickleObject = pickle.load(file)
                for x in  loaded_data.languages:
                    languages[x] = 1 

            self._initialize(vocab, loaded_data.seq_len, languages)

    def _initialize(self, vocab, seq_len, languages):
    ...
    def save_model_init_params(self, file_name):         
        obj = PickleObject(self.max_len, self.languages)
...
class PickleObject:
    def __init__(self, seq_len, languages):
        self.seq_len : int = seq_len
        self.languages : dict[str, int]= languages
